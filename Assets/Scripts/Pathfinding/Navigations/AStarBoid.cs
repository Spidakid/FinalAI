...
public class AStarBoid : MonoBehaviour
{
    ...
    private float intervalTime = 1.0f; //Interval time between path finding
    ...
    void Update()
    {
        ...
        if (!isStopped)
        {
            if (boidStats.isWithinOrigin)
            {
                Speed = initialSpeed;
            }
            else
            {
                Speed = this.initialSpeed + boidOriginStats.astarSpeedAdjust;
            }
    ...
    private void FollowPath()
    {
        ...
        else if (curNodeVector == goalPos.position && Vector3.Distance(this.transform.position, goalPos.position) < BoidOrigin.innerOriginRadius || Vector3.Distance(this.transform.position, goalPos.position) < BoidOrigin.innerOriginRadius)
        {
            if (!reachedGoal)
            {
                reachedGoal = true;
    ...
    void OnDrawGizmos()
    {
        if (showDebug && anim.GetBool("IsObstacleDetected"))
        {
            if (pathArray == null)
                return;
